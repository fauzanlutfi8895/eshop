...
const CustomProperties = ({ control, error }: any) => {
  //state untuk mengatur interaksi UI
    ...
    <div>
      <div className="flex flex-col gap-3">
        <Controller
          ...
          render={({ field }) => {
            //field.OnChange sudah membawa name=CustomProperties
            useEffect(() => {
              field.onChange(properties); //setiap ada perubahan (update) kali ini properties, lakukan ini
            }, [properties]);
              ...
              <div className="mt-2">
                ...
                {error?.custom_properties && (
                  <p className="text-red-500 text-xs mt-1">
                    {error.custom_properties.message as string}
                  </p>
                )}
              </div>
            );
