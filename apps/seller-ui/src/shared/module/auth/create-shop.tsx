...
const CreateShop = ({
  ...
  sellerId: string;
  setActiveStep: (step: number) => void; //karena set use state adalah sebuah fungsi untuk mengubah state
}) => {
  const {
    register,
    handleSubmit,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <input
          ...
          {...register("bio", {
            ...
            validate: (count) =>
              countWords(count) <= 100 ||
              "Bio cannot exceed more than 100 words",
          })}
        />
        {errors.bio && (
